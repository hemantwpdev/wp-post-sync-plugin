...
document.addEventListener('DOMContentLoaded', function () {
	...
	document.getElementById('add-target-btn').addEventListener('click', function () {
		...
		const message = document.getElementById('add-target-message');
		...
		jQuery.ajax({
			...
			success: function (response) {
				if (response.success) {
					...
					message.textContent = response.data || 'Failed to add target';
					message.className = 'error';
				}
			},
			error: function () {
				message.textContent = 'AJAX error';
				message.className = 'error';
			},
		});
	});
	...
	document.getElementById('save-settings-btn').addEventListener('click', function () {
		...
		if (mode === 'target') {
			data.target_key = document.getElementById('target_key').value;
			data.language = document.getElementById('language').value;
			data.chatgpt_key = document.getElementById('chatgpt_key').value;
		}
		...
		jQuery.ajax({
			...
			data: data,
			success: function (response) {
				message.textContent = response.data || 'Settings saved';
				message.className = 'success';
				setTimeout(() => {
					message.textContent = '';
					message.className = '';
